...
from xvfbUI.freenas import forms, models, utils
...
def start(request):
    xvfb_key, xvfb_secret = utils.get_xvfb_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=xvfb_key,
        secret=xvfb_secret)
    ...
    try:
        xvfb = models.Xvfb.objects.order_by('-id')[0]
        xvfb.enable = True
        xvfb.save()
    except IndexError:
        xvfb = models.Xvfb.objects.create(enable=True)

    try:
        form = forms.XvfbForm(xvfb.__dict__, instance=xvfb, jail=jail)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': 'Xvfb data did not validate, please configure it first.',
    ...
    cmd = "%s onestart" % utils.xvfb_control
    _popen = os.popen(cmd)
...
def stop(request):
    ...
    try:
        ...
        xvfb = models.Xvfb.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.xvfb_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
...
def edit(request):
    ...
    try:
        ...
        xvfb = models.Xvfb.objects.create()
    ...
    if request.method == "GET":
        form = forms.XvfbForm(instance=xvfb,
            jail=jail)
    ...
    form = forms.XvfbForm(request.POST,
        instance=xvfb,
        jail=jail)
    if form.is_valid():
        form.save()
        return JsonResponse(request, error=True, message="Xvfb settings successfully saved.")
...
def treemenu(request):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('xvfb_edit'),
        'kwargs': {'plugin_name': 'xvfb'},
    }
...
def treemenu_icon(request):
    with open(utils.xvfb_icon, 'rb') as f:
        icon = f.read()

    return HttpResponse(icon, content_type='image/png')

def popup_backgnd(request):
    with open(utils.xvfb_backgnd, 'rb') as f:
        backgnd = f.read()
